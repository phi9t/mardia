...
readonly CONTAINER_OUTPUT_ROOT="/workspace/outputs"
...
readonly DEFAULT_CONTAINER_IMAGE="sygaldry/zephyr:sglang"
readonly DEV_BUILDABLE_IMAGE="sygaldry/zephyr:base"
...
build_container_image() {
    ...
    local is_dev_buildable_image=false
    if [[ "${CONTAINER_IMAGE}" == "${DEFAULT_CONTAINER_IMAGE}" ]]; then
        is_default_image=true
    fi
    if [[ "${CONTAINER_IMAGE}" == "${DEV_BUILDABLE_IMAGE}" ]]; then
        is_dev_buildable_image=true
    fi
    ...
    if docker image inspect "${CONTAINER_IMAGE}" >/dev/null 2>&1; then
        if [[ "${BUILD_IMAGE_POLICY}" == "always" ]]; then
            build_image=true
        elif [[ "${BUILD_IMAGE_POLICY}" == "auto" && "${is_dev_buildable_image}" == "true" ]]; then
        ...
        if [[ "${BUILD_IMAGE_POLICY}" == "always" || "${is_dev_buildable_image}" == "true" ]]; then
            build_image=true
        else
