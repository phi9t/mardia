...
#   - skills/zephyr-mlsys-env/envs/<env>.yaml must exist
#
...
readonly ENVS_DIR="${PROJECT_ROOT}/skills/zephyr-mlsys-env/envs"
...
VERIFY=true
PUSH=false
REGISTRY="ghcr.io/phi9t/sygaldry"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --registry)
            REGISTRY="${2:-}"
            [[ -z "${REGISTRY}" ]] && error "Missing value for --registry"
            shift 2
            ;;
        --registry=*)
            REGISTRY="${1#*=}"
            ...
            ;;
        --help|-h)
            echo "Usage: $0 <env-name|all> [--dry-run] [--no-verify] [--push] [--registry REGISTRY]"
            echo ""
            ...
            echo "  --push             Push to registry after build+verify"
            echo "  --registry REG     Registry prefix (default: ghcr.io/phi9t/sygaldry)"
            echo ""
...
for env in "${BUILD_LIST[@]}"; do
    ...
    if [[ "${PUSH}" == "true" ]]; then
        ...
        if ! docker login "${REGISTRY_HOST}" --get-login >/dev/null 2>&1; then
            if [[ -n "${GITHUB_TOKEN:-}" ]] && [[ "${REGISTRY_HOST}" == "ghcr.io" ]]; then
