...
# ._view: unified package view directory (~417 MB)
COPY --from=spack_store --chown=kvothe:kvothe ._view /opt/spack_store/._view

# misc_cache: Spack metadata (~700 KB)
COPY --from=spack_store --chown=kvothe:kvothe misc_cache /opt/spack_store/misc_cache

# Create the view symlink (points to the hash-named directory under ._view)
RUN set -e; \
    VIEW_TARGET="$(ls -d /opt/spack_store/._view/*/ | head -1)"; \
    ln -sfn "${VIEW_TARGET%/}" /opt/spack_store/view; \
    echo "Spack view -> $(readlink /opt/spack_store/view)"
