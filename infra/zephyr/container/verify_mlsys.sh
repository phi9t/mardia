...
result="$(run_no_gpu 'test -x /opt/sygaldry/skills/zephyr-mlsys-env/scripts/uv-env-build.sh && echo yes')" || true
if [[ "${result}" == "yes" ]]; then
    pass "MLSys skill scripts baked at /opt/sygaldry/skills/zephyr-mlsys-env/"
else
...
result="$(run_no_gpu "
    ...
    /opt/sygaldry/skills/zephyr-mlsys-env/scripts/uv-env-validate.sh \
    ...
    if ! verify_has_nvidia_runtime; then
        ...
        case "${MLSYS_ENV}" in
            ...
            torchtitan)
                ...
                ;;
            megatronlm)
                import_check="from megatron.core import parallel_state; print(f'megatron-core ok')"
                ;;
            llm-serving-all)
