...
COPY --chown=kvothe:kvothe skills/zephyr-mlsys-env /opt/sygaldry/skills/zephyr-mlsys-env
RUN chmod +x /opt/sygaldry/skills/zephyr-mlsys-env/scripts/*.sh
...
RUN --mount=type=cache,target=/opt/uv_cache,uid=1000,gid=1000 \
    ...
    VENV_ROOT="${VENV_ROOT}" SKIP_VALIDATE=1 \
        /opt/sygaldry/skills/zephyr-mlsys-env/scripts/uv-env-build.sh \
